...
import { ArrowLeft, User, KeyRound, Mail, Loader2, Sparkles } from "lucide-react";
...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-[#030308] flex flex-col justify-center relative overflow-hidden font-sans text-white">
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-blue-600/10 rounded-full blur-[150px] pointer-events-none mix-blend-screen animate-pulse" />
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-fuchsia-600/10 rounded-full blur-[120px] pointer-events-none mix-blend-screen" />
        ...
        >
          <div className="bg-[#05050A]/80 backdrop-blur-3xl border border-white/10 p-10 rounded-[2rem] shadow-[0_0_50px_rgba(0,0,0,0.8)] relative overflow-hidden">
            ...
            <div className="text-center mb-10 relative z-10">
              <div className="w-16 h-16 bg-blue-500/10 rounded-2xl flex items-center justify-center mx-auto mb-6 border border-blue-500/20 shadow-[0_0_20px_rgba(59,130,246,0.3)]"><Sparkles className="w-8 h-8 text-blue-400" /></div>
              <h2 className="text-3xl font-black text-white mb-2 tracking-tight">Create Account</h2>
              <p className="text-slate-400 font-medium">Join to instantly sequence matching benefits.</p>
            </div>
            ...
            <form onSubmit={handleRegister} className="space-y-6 relative z-10">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none"><User className="w-5 h-5 text-slate-500" /></div>
                  <input type="text" required onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full bg-black/50 border border-white/10 rounded-2xl pl-14 pr-4 py-4 text-white focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-inner font-medium placeholder:text-slate-600" placeholder="Aditi Sharma" />
                </div>
              </div>
              ...
              <button type="submit" disabled={isLoading} className="w-full mt-8 bg-blue-600 hover:bg-blue-500 text-white font-black uppercase tracking-widest rounded-2xl px-4 py-5 transition-all shadow-[0_0_30px_rgba(37,99,235,0.4)] disabled:opacity-70 flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-[0.98]">
                {isLoading ? <><Loader2 className="w-5 h-5 animate-spin" /> Provisioning Node...</> : "Execute Protocol"}
              </button>
            </form>

            <p className="text-center text-slate-400 mt-10 text-sm font-medium relative z-10">
              Already have an account? <Link href="/login" className="text-blue-400 hover:text-blue-300 font-black transition-colors ml-1">Authenticate here</Link>
            </p>
          </div>
        </motion.div>
      </div>
