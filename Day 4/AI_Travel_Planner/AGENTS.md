...
### Mandatory: Travel before writing CrewAI code
...
travel planninger:
  role: >
    {topic} Senior Data Traveler
  goal: >
    Uncover cutting-edge developments in {topic}
  backstory: >
    You're a seasoned travel planninger with a knack for uncovering
...
### tasks.yaml
```yaml
travel planning_task:
  description: >
    Conduct thorough travel planning about {topic}.
  ...
  agent: travel planninger
  # Optional:
  # tools: [search_tool]
  # output_file: output/travel planning.md
  # markdown: true
  # async_execution: false

writing_task:
  description: >
    Write an article based on the travel planning findings about {topic}.
  expected_output: >
...
class TravelCrew:
    ...
    @agent
    def travel planninger(self) -> Agent:
        return Agent(
            config=self.agents_config["travel planninger"],  # type: ignore[index]
            tools=[SerperDevTool()],
            verbose=True,
        )
    ...
    @task
    def travel planning_task(self) -> Task:
        return Task(
            config=self.tasks_config["travel planning_task"],  # type: ignore[index]
        )
...
```python
#!/usr/bin/env python
from my_crew.crew import TravelCrew
...
class TravelFindings(BaseModel):
    main_points: list[str]
    key_technologies: list[str]
    future_predictions: str

travel planninger = Agent(
    role="AI Traveler",
    goal="Travel the latest AI developments",
    backstory="Expert AI travel planninger...",
    tools=[SerperDevTool()],
    verbose=True,
)

# Unstructured output
result = travel planninger.kickoff("What are the latest LLM developments?")
print(result.raw)           # str
print(result.agent_role)    # "AI Traveler"
print(result.usage_metrics) # token usage

# Structured output with response_format
result = travel planninger.kickoff(
    "Summarize latest AI developments",
    response_format=TravelFindings,
)
print(result.pydantic.main_points)  # List[str]

# Async variant
result = await travel planninger.kickoff_async("Your query", response_format=TravelFindings)
```
...
def analysis_task(self) -> Task:
    return Task(
        ...
        context=[self.travel planning_task()],  # Gets output from travel planning_task
    )
```
...
class TravelFlow(Flow[TravelState]):
    ...
    @listen(set_topic)
    def do_travel planning(self):
        # self.state.topic is available
        result = TravelCrew().crew().kickoff(
            inputs={"topic": self.state.topic}
        )
        self.state.travel planning = result.raw
```
...
```python
from crewai.flow.flow import Flow, listen, start
from my_project.crews.travel planning_crew.travel planning_crew import TravelCrew
from my_project.crews.writing_crew.writing_crew import WritingCrew

class ContentFlow(Flow[ContentState]):
    @start()
    def travel planning(self):
        result = TravelCrew().crew().kickoff(
            inputs={"topic": self.state.topic}
        )
        self.state.travel planning = result.raw

    @listen(travel planning)
    def write(self):
        result = WritingCrew().crew().kickoff(
            inputs={
                "topic": self.state.topic,
                "travel planning": self.state.travel planning,
            }
        )
